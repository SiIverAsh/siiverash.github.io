...
def process_lifecycle():
    ...
    for filename in os.listdir(POSTS_DIR):
        ...
        if not fm_date or "UPLOAD_TIME" in fm_date or "2026-01-01" in fm_date:
            ...
            fm_date = now_str
            fm_mod = now_str
            print(f"🆕 [发布初始化] {filename} (已锁定发布日期)")
            needs_update = True
        ...
        if committed_body is not None and current_body != committed_body:
            # 正文内容确实发生了变动
            front_matter["last_modified_at"] = now_str
            print(f"📝 [正文内容更新] {filename} (检测到正文变动，已更新 last_modified_at)")
            needs_update = True
        elif not fm_mod or fm_mod != fm_date:
            # 数据归位：内容没变，但更新时间字段缺失或异常，将其设为等于发布时间（隐藏前端图标）
            front_matter["last_modified_at"] = fm_date
            print(f"🧹 [日期规范化] {filename} (补齐缺失字段或对齐时间，已隐藏前端更新图标)")
            needs_update = True
